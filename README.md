...
**macOS**: Grant Bluetooth access when prompted. **The H10 may not show in System Settings → Bluetooth** when you use this app—that’s normal. The app connects via BLE without going through the system pairing screen, so the device often won’t appear in the list and you won’t get a pairing prompt. **Connection issues?** Try turning Bluetooth off and back on in System Settings → Bluetooth, or reboot your Mac (macOS Bluetooth can get stuck). **If you get "Streaming RR intervals" but no data:** (1) Fully quit Elite HRV and any other app that uses the H10, then run `./run.sh` again. (2) If it still doesn’t work, **reset the H10** so it clears old pairing state: remove the battery for 30 seconds, put it back in, strap on with moistened electrodes, then run `./run.sh`. (3) You can also try **Forget This Device** for the H10 in System Settings → Bluetooth (if it’s listed), then run again. If you see **"Peer removed pairing information"**, forget the H10 in Bluetooth, then run again.

**"No Polar H10 found"?** The script retries scanning several times (see `--scan-retries`). Between attempts it prints tips: strap on and moisten the electrodes, quit Polar Beat (or any app using the H10), and ensure Bluetooth is on. If you use a specific device name, try without `--device` once to see if any H10 appears. **If connection is flaky or the H10 never shows up:** try turning Bluetooth off and back on in System Settings → Bluetooth, or reboot your Mac—macOS Bluetooth can get into a stuck state and a reset often fixes it.
